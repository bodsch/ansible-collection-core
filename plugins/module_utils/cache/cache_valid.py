...
def cache_valid_old(module, cache_file_name, cache_minutes=60, cache_file_remove=True) -> bool:
    """
...
def cache_valid(module: Any, cache_file_name: str, cache_minutes: int = 60, cache_file_remove: bool = True) -> bool:
    """
    ...
    try:
        ...
        module.debug(msg=f"Fehler beim Lesen der Modifikationszeit von '{cache_file_name}': {e} → Cache ungültig")
        return True
    ...
    module.debug(msg=f"Cache-Datei '{cache_file_name}' gefunden. Letzte Änderung: {modification_time.isoformat()}")
    module.debug(msg=f"  → Jetzt:       {now.isoformat()}")
    module.debug(msg=f"  → Alter:       {diff_minutes:.2f} Minuten (Limit: {cache_minutes} Minuten)")
    ...
    if is_expired and cache_file_remove:
        try:
            path.unlink()
            module.debug(msg=f"  → Alte Cache-Datei '{cache_file_name}' wurde gelöscht.")
        except OSError as e:
            module.debug(msg=f"  → Fehler beim Löschen der Cache-Datei '{cache_file_name}': {e}")
