...
---
module: openvpn_crl
short_description: Manage and inspect an OpenVPN certificate revocation list.
version_added: "1.1.3"
author:
  - "Bodo Schulz (@bodsch) <me+ansible@bodsch.me>"

description:
  - Inspect an existing OpenVPN certificate revocation list generated by Easy-RSA.
  - Parse CRL metadata such as C(last_update) and C(next_update).
  - Optionally return revoked certificate entries contained in the CRL.
  - Renew the CRL by removing the current C(crl.pem) and invoking Easy-RSA
    C(gen-crl).

requirements:
  - easyrsa
  - OpenSSL libraries and tools as required by the collection crypto utilities

notes:
  - Check mode is not supported.
  - The O(force) parameter is currently reserved for compatibility and is not
...
options:
  state:
    description:
      ...
      - Use C(status) to inspect the current CRL.
      - Use C(renew) to regenerate the CRL using Easy-RSA.
    type: str
    default: status
    choices:
      - status
  ...
  pki_dir:
    description:
      ...
      - The module expects the CRL file at C(<pki_dir>/crl.pem).
    type: str
  ...
  working_dir:
    description:
      ...
      - Useful when Easy-RSA must be executed from a specific directory.
    type: path
    required: false

  list_revoked_certificates:
    description:
      - Include the parsed list of revoked certificates in the module result.
    type: bool
    default: false

  warn_for_expire:
    description:
      - Evaluate whether the CRL is near expiry based on O(expire_in_days).
      - Adds C(expired) to the result.
      - Adds C(warn=true) when the CRL is considered near expiry.
    type: bool
    default: true

  expire_in_days:
    description:
      - Threshold in days used to determine whether the CRL is near expiry.
      - If C(next_update - now <= expire_in_days), the module returns
        C(expired=true).
    type: int
    default: 10
...
- name: Warn if the CRL expires within 14 days
  bodsch.core.openvpn_crl:
    state: status
    pki_dir: /etc/easy-rsa/pki
...
- name: Renew the CRL using Easy-RSA
  bodsch.core.openvpn_crl:
    state: renew
    pki_dir: /etc/easy-rsa/pki
...
- name: Show revoked certificates
  ansible.builtin.debug:
    var: crl_status.revoked_certificates
...
msg:
  ...
  returned: on failure or when state=renew
  type: str
  sample: "CRL renewed successfully."

changed:
  description:
    - Indicates whether the module changed the managed host.
    - This is only relevant for C(state=renew).
...
last_update:
  ...
  returned: when state=status and the CRL could be parsed
  type: dict
  contains:
    raw:
      description:
        - Raw value extracted from the CRL.
      returned: always
      type: str
    parsed:
      description:
        - Parsed representation returned by the collection helper.
      returned: always
      type: raw
...
expired:
  description:
    - Indicates whether the CRL expires within O(expire_in_days).
...
warn:
  ...
  returned: when state=status and warn_for_expire=true and expired=true
  type: bool
  sample: true
...
class OpenVPNCrl:
    """
    Manage inspection and renewal of an Easy-RSA-generated CRL.

    The class reads module parameters, parses CRL metadata, optionally exposes
    revoked certificate entries, and can regenerate the CRL by delegating to
    the EasyRSA helper implementation.
    """

    def __init__(self, module):
        """
        Initialize the CRL manager with module parameters.

        Args:
            module: Active Ansible module instance that provides parameters,
                logging, and result handling.
        """
        self.module = module
        self.module.log("OpenVPNCrl::__init__()")
    ...
    def run(self):
        """
        Execute the requested module action.

        Depending on the configured state, this method either inspects the
        current CRL file or regenerates it through Easy-RSA.

        Returns:
            dict: Module result containing the operation outcome.
        """
        self.module.log("OpenVPNCrl::run()")
    ...
    def test_crl(self):
        """
        Read and parse the current certificate revocation list.

        The method loads the CRL file from disk, extracts metadata using the
        crypto helper utilities, and optionally includes revoked certificate
        entries in the result.

        Returns:
            dict: Result dictionary containing CRL metadata or an error message.
        """
        self.module.log("OpenVPNCrl::test_crl()")
    ...
    def renew_crl(self):
        """
        Regenerate the CRL file using Easy-RSA.

        If configured, the working directory is changed before executing the
        Easy-RSA command. An existing CRL file is removed before regeneration.

        Returns:
            dict: Result dictionary containing the renewal status.
        """
        self.module.log("OpenVPNCrl::renew_crl()")
    ...
    def expired(self, next_update):
        """
        Determine whether the CRL is near expiry.

        The CRL is considered near expiry when the number of days until
        ``next_update`` is less than or equal to the configured threshold.

        Args:
            next_update: Parsed CRL next-update timestamp.

        Returns:
            bool: C(True) when the CRL is near expiry, otherwise C(False).
        """
        self.module.log(f"OpenVPNCrl::expired(next_update: {next_update})")
...
def main():
    """
    Create the Ansible module instance and execute the CRL workflow.

    The function defines the module argument specification, initializes the
    module wrapper class, runs the requested action, and returns the final
    result to Ansible.
    """
    args = dict(
